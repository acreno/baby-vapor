import React from 'react';

export interface OutlineChatBubbleUpProps {
  className?: string;
  size?: number | string;
  color?: string;
  style?: React.CSSProperties;
}

export const OutlineChatBubbleUp: React.FC<OutlineChatBubbleUpProps> = ({ 
  className = '', 
  size = 24, 
  color = 'currentColor',
  style,
  ...props 
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    style={{ color, ...style }}
    {...props}
  >
    <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M9.06059 4.2451C8.91836 4.08707 8.71573 3.99683 8.50312 3.99683C8.2905 3.99683 8.08788 4.08707 7.94565 4.2451L5.69566 6.7451C5.41857 7.05299 5.44353 7.52721 5.75141 7.8043C6.0593 8.08139 6.53351 8.05643 6.81061 7.74855L7.75313 6.70129L7.75314 10.2531C7.75314 10.6673 8.08892 11.0031 8.50314 11.0031C8.91735 11.0031 9.25314 10.6673 9.25314 10.2531L9.25313 6.7013L10.1957 7.74855C10.4728 8.05643 10.947 8.08139 11.2549 7.80429C11.5627 7.5272 11.5877 7.05298 11.3106 6.7451L9.06059 4.2451Z" />
    <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M1.99762 7.49822C1.99762 3.92058 4.91787 1 8.49576 1C12.073 1 14.9976 3.91995 14.9976 7.49822C14.9976 11.0759 12.0736 14 8.49576 14C7.46633 14 6.4723 13.7523 5.60393 13.3235L3.35068 13.9609L3.34849 13.9615C2.90455 14.0856 2.51147 13.8947 2.28589 13.6447C2.06499 13.4 1.9293 13.022 2.03837 12.6252L2.65158 10.3384C2.22288 9.47381 1.99762 8.52557 1.99762 7.49822ZM8.49576 2.5C5.74636 2.5 3.49762 4.74895 3.49762 7.49822C3.49762 8.38281 3.70976 9.17178 4.10698 9.88384C4.20204 10.0543 4.22694 10.255 4.1764 10.4435L3.67587 12.31L5.48316 11.7988C5.67419 11.7447 5.87886 11.7687 6.0523 11.8653C6.76891 12.2645 7.61757 12.5 8.49576 12.5C11.2452 12.5 13.4976 10.2475 13.4976 7.49822C13.4976 4.74958 11.2458 2.5 8.49576 2.5Z" />
  </svg>
);

OutlineChatBubbleUp.displayName = 'OutlineChatBubbleUp';

export default OutlineChatBubbleUp;