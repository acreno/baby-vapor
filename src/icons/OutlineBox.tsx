import React from 'react';

export interface OutlineBoxProps {
  className?: string;
  size?: number | string;
  color?: string;
  style?: React.CSSProperties;
}

export const OutlineBox: React.FC<OutlineBoxProps> = ({ 
  className = '', 
  size = 24, 
  color = 'currentColor',
  style,
  ...props 
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    style={{ color, ...style }}
    {...props}
  >
    <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M4.98975 3.5C4.91251 3.5 4.8396 3.5357 4.79225 3.59672L3.70318 5H12.2999L11.2083 3.59651C11.1609 3.53562 11.0881 3.5 11.0109 3.5H4.98975ZM3.50381 12.25V6.5H12.5V12.25C12.5 12.3881 12.3881 12.5 12.25 12.5H3.75381C3.61574 12.5 3.50381 12.3881 3.50381 12.25ZM3.60725 2.67706C3.93874 2.24992 4.44906 2 4.98975 2H11.0109C11.551 2 12.0607 2.24933 12.3923 2.6756L13.6314 4.26871C13.8703 4.57589 14 4.95395 14 5.34311V12.25C14 13.2165 13.2165 14 12.25 14H3.75381C2.78732 14 2.00381 13.2165 2.00381 12.25V5.34252C2.00381 4.95399 2.13311 4.57652 2.37132 4.26958L3.60725 2.67706ZM9.25 9.5C9.66421 9.5 10 9.16421 10 8.75C10 8.33579 9.66421 8 9.25 8H6.75C6.33579 8 6 8.33579 6 8.75C6 9.16421 6.33579 9.5 6.75 9.5H9.25Z" />
  </svg>
);

OutlineBox.displayName = 'OutlineBox';

export default OutlineBox;