import React from 'react';

export interface OutlineChatBubbleDownProps {
  className?: string;
  size?: number | string;
  color?: string;
  style?: React.CSSProperties;
}

export const OutlineChatBubbleDown: React.FC<OutlineChatBubbleDownProps> = ({ 
  className = '', 
  size = 24, 
  color = 'currentColor',
  style,
  ...props 
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    style={{ color, ...style }}
    {...props}
  >
    <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M7.75312 8.29886L7.75312 4.74707C7.75312 4.33286 8.08891 3.99707 8.50312 3.99707C8.91734 3.99707 9.25312 4.33286 9.25312 4.74707L9.25312 8.29887L10.1957 7.25161C10.4727 6.94373 10.947 6.91877 11.2548 7.19586C11.5627 7.47295 11.5877 7.94717 11.3106 8.25505L9.06061 10.7551C8.91838 10.9131 8.71576 11.0033 8.50314 11.0033C8.29053 11.0033 8.0879 10.9131 7.94567 10.7551L5.69565 8.25506C5.41856 7.94718 5.44352 7.47296 5.7514 7.19586C6.05928 6.91877 6.5335 6.94372 6.81059 7.25161L7.75312 8.29886Z" />
    <path fill="currentColor" fill-rule="evenodd" clip-rule="evenodd" d="M1.99762 7.49822C1.99762 3.92058 4.91787 1 8.49576 1C12.073 1 14.9976 3.91995 14.9976 7.49822C14.9976 11.0759 12.0736 14 8.49576 14C7.46633 14 6.4723 13.7523 5.60393 13.3235L3.35068 13.9609L3.34849 13.9615C2.90455 14.0856 2.51147 13.8947 2.28589 13.6447C2.06499 13.4 1.9293 13.022 2.03837 12.6252L2.65158 10.3384C2.22288 9.47381 1.99762 8.52557 1.99762 7.49822ZM8.49576 2.5C5.74636 2.5 3.49762 4.74895 3.49762 7.49822C3.49762 8.38281 3.70976 9.17178 4.10698 9.88384C4.20204 10.0543 4.22694 10.255 4.1764 10.4435L3.67587 12.31L5.48316 11.7988C5.67419 11.7447 5.87886 11.7687 6.0523 11.8653C6.76891 12.2645 7.61757 12.5 8.49576 12.5C11.2452 12.5 13.4976 10.2475 13.4976 7.49822C13.4976 4.74958 11.2458 2.5 8.49576 2.5Z" />
  </svg>
);

OutlineChatBubbleDown.displayName = 'OutlineChatBubbleDown';

export default OutlineChatBubbleDown;